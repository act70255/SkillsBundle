---
description: 一次完成 Vitest 測試的 plan -> genscript -> execute 總流程
---
...
- 針對 $ARGUMENTS 串接執行 Vitest 三階段流程：
  1) `/workflow-testing-plan-vitest`
  2) `/workflow-testing-genscript-vitest`
  3) `/workflow-testing-execute-vitest`
- 本指令為總控流程；使用者只需呼叫一次 `/workflow-testing-vitest`，後續子流程由本流程依序調用

建議輸入格式：

- `<測試範圍> [--src-path <path>] [--test-path <path>] [--artifact-dir <path>]`
- 無參數模式可直接執行 `/workflow-testing-vitest`，但若缺 `測試範圍`，需先向使用者確認
...
1. 先做 `Preflight Input Check`，彙整本次最小必要資訊：
   ...
   - `test-path`（可省略，預設 `testscripts/vitest`）
   - `artifact-dir`（可省略，預設 `testing-artifacts/vitest`）
2. 若缺少必要資訊（至少包含 `測試範圍`），不得直接執行子流程；需先向使用者逐項確認與補齊
...
2. 可選項在必要項確認後，需先統一詢問「是否要指定」：
   ...
   - `--test-path`（未指定則用 `testscripts/vitest`）
   - `--artifact-dir`（未指定則用 `testing-artifacts/vitest`）
3. 若使用者回答不指定可選項，需明確回報將套用的預設值與略過項目

測試方式差異（Vitest，規劃必看）：

1. 以 `src` 靜態分析為主，重點是 unit/integration，不依賴站台 URL
2. 案例需優先覆蓋函式/元件邏輯、邊界條件、錯誤處理、Mock 隔離
3. `plan` 階段需明確區分哪些案例可 `AutomationTarget=vitest`，哪些保留 `manual`
...
3. `plan` 完成後，需確認下列最小輸入已存在，再進入 `genscript`：
   - `testing-artifacts/vitest/TEST-PLAN.md`（或 `--artifact-dir/TEST-PLAN.md`）
   - `testing-artifacts/vitest/TEST-CASES.md`（或 `--artifact-dir/TEST-CASES.md`）
   - `testing-artifacts/vitest/ACCEPTANCE-CRITERIA.md`（或 `--artifact-dir/ACCEPTANCE-CRITERIA.md`）
4. `genscript` 完成後，需確認已產出：
   - Vitest 腳本（`testscripts/vitest/` 或 `--test-path`）
   - `testing-artifacts/vitest/TEST-SCRIPT-REPORT.md`（或 `--artifact-dir/TEST-SCRIPT-REPORT.md`）
5. `execute` 階段需依子流程規範執行 Vitest（`yarn vitest run ... --environment jsdom`）
...
1. 若 `PRECHECK` 缺 `測試範圍` -> 停在 `PRECHECK`，先補件
...
4. 若 `execute` 找不到可執行測試（`--test-path` 無檔案且預設規則無匹配）-> 轉 `BLOCKED`（測試資產阻塞）
5. 若 `execute` 失敗且可修復 -> 修復後只重跑 `execute`；不可修復 -> 轉 `BLOCKED`

輸出（強制）：

1. `testing-artifacts/vitest/WORKFLOW-RUN-REPORT.md`（或 `--artifact-dir/WORKFLOW-RUN-REPORT.md`）
   - 本次輸入參數快照
   - 三階段執行結果（Pass/Fail/Blocked）
   - 各階段關鍵產物路徑
   - 若中斷，需記錄中斷階段、原因與下一步建議
2. 若 execute 有阻塞，沿用 `testing-artifacts/vitest/[TEST]BLOCKERS.md`（或 `--artifact-dir/[TEST]BLOCKERS.md`）
...
- `InputSnapshot`: `scope/srcPath/testPath/artifactDir`
...
- 需要補齊測試規劃或 AC 基線：回到 `/workflow-testing-plan-vitest`
- 需要補齊或重生腳本：回到 `/workflow-testing-genscript-vitest`
- 僅需重跑驗證：直接執行 `/workflow-testing-execute-vitest`
