...
- 無參數模式：可直接執行 `/workflow-testing-plan-playwright`；未提供 `<變更範圍>` 與 `--src-path` 時，預設掃描 `src/`，若 `src/` 不存在則掃描工作區根目錄
...
1. 一律讀取目標 `src` 範圍與路由/端點進行靜態分析（頁面流程、主要互動、導頁與 API 依賴），作為測試規劃基線；若未提供路徑則使用參數規範中的預設路徑
2. 讀取並對齊 `SPEC.md`、`ACCEPTANCE-CRITERIA.md`、`TASKS.md`（若存在），用於需求追溯與優先級校正
3. 若文件與 `src` 行為不一致，需在 `testing-artifacts/playwright/INPUT-GAP.md`（或 `--artifact-dir/INPUT-GAP.md`）標註差異、風險與建議修正方向
...
4. 本流程不產生測試腳本；若需產碼，改用 `/workflow-testing-script-generate-playwright`
...
4. `testing-artifacts/playwright/INPUT-GAP.md`（條件式，文件缺失或與 `src` 行為不一致時；或 `--artifact-dir/INPUT-GAP.md`）
   - 缺失文件清單
   - 以 `src`/路由推導出的假設與風險
