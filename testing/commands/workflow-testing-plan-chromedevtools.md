---
description: 建立 Chrome DevTools 測試策略、測試案例與品質閘
---
...
- 針對 $ARGUMENTS 建立可執行、可追溯的 Chrome DevTools 測試規劃。
...
3. 若文件與 `src` 行為不一致，需在 `testing-artifacts/chromedevtools/INPUT-GAP.md`（或 `--artifact-dir/INPUT-GAP.md`）標註差異、風險與建議修正方向
...
1. 僅做測試規劃，不可修改產品程式碼
2. 測試案例需標註 `AutomationTarget = chromedevtools | manual`
3. 測試案例需可追溯到 AC 與 TASK 編號；若文件缺失，允許使用 `INFERRED-*` 編號
4. 不在本流程產出測試腳本；若需產碼，改用 `/workflow-testing-script-generate-chromedevtools`

請輸出（強制）：

1. `testing-artifacts/chromedevtools/TEST-PLAN.md`（或 `--artifact-dir/TEST-PLAN.md`）
   - 測試範圍（integration/e2e）
   - 優先測試路徑與風險分級
   - 品質閘條件（G1~G4）
   - 建議執行順序
2. `testing-artifacts/chromedevtools/TEST-CASES.md`（或 `--artifact-dir/TEST-CASES.md`）
   - 測試案例清單（前置條件、步驟、預期結果）
   - 每個案例的 AC/TASK Trace
   - 每個案例的 AutomationTarget
3. `testing-artifacts/chromedevtools/ACCEPTANCE-CRITERIA.md`（條件式，僅原檔缺失時；或 `--artifact-dir/ACCEPTANCE-CRITERIA.md`）
   - 以 `INFERRED-AC-*` 建立可追溯 AC 基線
   - 每條 AC 需對應到 `testing-artifacts/chromedevtools/TEST-CASES.md`（或 `--artifact-dir/TEST-CASES.md`）的至少一個 Case
   - 標註 `Status: Inferred` 與待產品/規格確認項目
4. `testing-artifacts/chromedevtools/INPUT-GAP.md`（條件式，文件缺失或與 `src` 行為不一致時；或 `--artifact-dir/INPUT-GAP.md`）
   - 缺失文件清單
   - 以 `src` 推導出的假設與風險
