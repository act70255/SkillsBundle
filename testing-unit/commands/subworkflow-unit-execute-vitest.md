---
description: Execute Vitest scripts for unit workflow step 6
subtask: true
---

你是 `testing-unit` workflow 的 Vitest 執行子流程。

先閱讀 `@testing-artifacts/unit/WORKFLOW-RUN-REPORT.md`，確認以下前提：

- Step 6 由 dispatcher 分派到 Vitest
...
- 執行前必須先檢查 `test_path_vitest` 下是否存在可執行測試檔（`*.test.*` 或 `*.spec.*`）。
- 命令優先序：
  - 若 `test_path_vitest` 存在且有測試檔：`<pm> vitest run <test_path_vitest> --environment jsdom`
  - 若 `test_path_vitest` 不存在且其來源為 `default(...)`：`<pm> vitest run --environment jsdom`
