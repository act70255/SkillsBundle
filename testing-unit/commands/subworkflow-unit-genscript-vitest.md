---
description: Generate Vitest scripts for unit workflow step 5
subtask: true
---

你是 `testing-unit` workflow 的 Vitest 腳本產生子流程。

先閱讀 `@testing-artifacts/unit/WORKFLOW-RUN-REPORT.md`，確認以下前提：

- Step 5 由 dispatcher 分派到 Vitest
...
- 寫入：`testscripts/vitest/`（或 `test_path_vitest`）與 `testing-artifacts/unit/TEST-SCRIPT-REPORT.md`
- 寫入前必須先確認 `test_path_vitest` 目錄存在；若不存在需先建立再寫入。
- 只產生 `AutomationTarget=vitest` 的案例。
- 每個測試名稱需包含 Case ID（例如 `UTC-001`）。
- 測試檔命名遵循 `*.test.{js,ts}` 或 `*.spec.{js,ts}`。
- 可新增/更新測試腳本，不可修改產品程式碼。
- 若缺 `TEST-PLAN.md` 或 `TEST-CASES.md`，停止並提示先回 Step 3。
- 若無任何 `AutomationTarget=vitest` 案例，需在 `TEST-SCRIPT-REPORT.md` 記錄 skipped 原因。
