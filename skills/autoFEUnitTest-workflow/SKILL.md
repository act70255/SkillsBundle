...
- Input gate:
  ...
  - `test_env` 屬於 Step 1 的自動修復型輸入；不得因其他 required inputs 缺失而延後偵測或初始化
- Re-validation gate:
  - Step 2 補件完成後，必須回到 Step 1 重跑輸入驗證，不得直接跳到 Step 3
- Strategy gate:
...
- 若 Step 2 已完成補件，續跑時必須先回到 Step 1 重跑輸入驗證
...
- 專案預設： [project-profile-auto-fe-unit.md](project-profile-auto-fe-unit.md)
