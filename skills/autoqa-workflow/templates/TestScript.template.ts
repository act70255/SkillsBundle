import fs from 'node:fs';
import path from 'node:path';
import { test, expect, type Page } from '@playwright/test';

function getBaseUrl(): string {
  const runtimeTargetUrl = process.env.WORKFLOW_TARGET_URL?.trim();
  if (runtimeTargetUrl) {
    return runtimeTargetUrl;
  }

  const runReportPath = path.resolve(process.cwd(), 'testing-artifact/handoff/RunReport.md');
  if (!fs.existsSync(runReportPath)) {
    throw new Error('Missing testing-artifact/handoff/RunReport.md');
  }

  const runReport = fs.readFileSync(runReportPath, 'utf8');
  const match = runReport.match(/^-\s*Target URL:\s*(.+)$/m);
  const baseUrl = match?.[1]?.trim();

  if (!baseUrl) {
    throw new Error('Missing Target URL in RunReport');
  }

  return baseUrl;
}

async function openEntryPage(page: Page): Promise<void> {
  await page.goto(getBaseUrl());
  await expect(page.locator('body')).toBeVisible();
}

test.describe('workflow-e2e generated suite', () => {
  test.beforeEach(async ({ page }) => {
    await openEntryPage(page);
  });

  test('TC-001 replace with documented happy path', async ({ page }) => {
    await expect(page).toHaveURL(/.+/);
    await expect(page.locator('body')).toBeVisible();
  });

  test('TC-002 replace with documented negative or boundary path', async ({ page }) => {
    await expect(page.locator('body')).toBeVisible();
  });
});
