...
此目錄提供一套以 OpenCode custom commands 為核心的固定式 e2e workflow，目標是把文件分析、登入判定、站台掃描、測試設計、測試執行與最終報告拆成可續跑、可阻塞、可追蹤的流程。
...
- 將文件分析、SRC 掃描、登入驗證、站台掃描、測試案例、測試腳本與測試報告串成可追溯鏈條。
...
- `.opencode/template/SrcScanSummary.template.md`: Step 4 SRC 掃描摘要模板。
- `.opencode/template/SiteScanSummary.template.md`: Step 6 站台掃描基線模板。
- `.opencode/template/ExecutionSummary.template.md`: Step 8 執行摘要模板。
- `.opencode/template/TestScript.template.ts`: Step 7 Playwright 測試腳本模板。
- `.opencode/template/TestPlan.template.md`: Step 7 測試計畫模板。
- `.opencode/template/TestCases.template.md`: Step 7 測試案例模板。
- `.opencode/template/TestReport.template.md`: Step 9 測試報告模板。
- `.opencode/template/.env.playwright.template`: 目標站台需要登入時，用來產出本次 workflow Playwright 環境檔的模板。
- `.opencode/template/PlaywrightEnvLoader.template.cjs`: Step 5 / Step 8 需要登入時建立共用 CJS preload env loader 的模板。
...
| 4 | 掃描 `source_code_path` 內的 SRC（若有提供） | `testing-artifact/handoff/SrcScanSummary.md` |
| 5 | 依文件、SRC 與輕量 Playwright 探測判定是否需要登入；若需要則於同一步內驗證登入條件 | `RunReport` 的登入判定與 auth metadata |
| 6 | 使用 Playwright 依文件掃描站台頁面 | `testing-artifact/handoff/SiteScanSummary.md` |
| 7 | 產出測試計畫、測試案例與測試腳本 | `testing-artifact/deliverables/TestPlan.md`、`testing-artifact/deliverables/TestCases.md`、`testing-artifact/scripts/TestScript.ts` |
| 8 | 執行測試腳本 | `testing-artifact/handoff/ExecutionSummary.md` |
| 9 | 產出測試報告 | `testing-artifact/deliverables/TestReport.md` |
...
- 若檔案不存在，視為新工作，先確認並建立 `testing-artifact/handoff/`、`testing-artifact/deliverables/` 與 `testing-artifact/scripts/`，再以 `.opencode/template/RunReport.template.md` 建立 `testing-artifact/handoff/RunReport.md`，最後從 Step 1 開始。
...
若第 5 步判定網站需要登入，流程必須立刻在同一步內呼叫 `/subworkflow-e2e-auth`：

1. 若 `testing-artifact/deliverables/.env.playwright` 不存在，由 `.opencode/template/.env.playwright.template` 產出
2. 若 `testing-artifact/scripts/playwright-env-loader.cjs` 不存在，由 `.opencode/template/PlaywrightEnvLoader.template.cjs` 產出
3. 驗證 `.env.playwright` 是否可在執行期成功載入並滿足登入條件
4. 若缺少憑證或登入驗證失敗，將狀態設為 `BLOCKED`
5. 保持 `Current Step: Step 5`，等待使用者補齊後續跑
...
- Step 5 成功完成後必須直接進入 Step 6；Step 6 成功完成後必須直接進入 Step 7；Step 7 成功完成後必須直接進入 Step 8；Step 8 成功完成後必須直接進入 Step 9。
- Step 9 完成後應直接結束為 `DONE`，不再等待額外確認。
- 若 `RunReport` 已記錄 `Requires Login: true` 且 Step 5 checklist 尚未完成，恢復執行時不得重做 Step 3-4 或重新判定登入需求，而是直接接續 `/subworkflow-e2e-auth`。
...
- 若 `source_code_path = none`，必須在 `SrcScanSummary.md` 明確記錄 skipped 與原因，但仍視為 Step 4 完成。
- 若 `source_code_path` 有提供，必須至少整理：路由或頁面對應、角色或權限判斷邏輯、驗證規則、核心流程分支、錯誤處理、測試資料線索、可自動化測試節點。
- 若發現與文件基線不一致，必須記錄在 `SrcScanSummary.md`，供 Step 5-7 使用。

## Step 5 規則

- Step 5 可以拆成兩段理解：先判定「是否需要登入」，若答案是需要，再於同一步內驗證「是否已滿足登入條件」。
...
- 只有在文件、SRC 與頁面訊號仍無法明確判定，或三者互相矛盾時，才向使用者確認。
- 若判定 `Requires Login = false`，必須在 `RunReport` 回寫 `Env Validation Status (Last Check): not_required`，並直接完成 Step 5。
...
- `/subworkflow-e2e-auth` 沒有自己的 step 編號；它是 Step 5 與 Step 8 內共用的登入條件驗證 helper。

## Auth helper 快速理解

- `/subworkflow-e2e-auth` 不是新的 workflow step；它只是 Step 5 與 Step 8 內部會呼叫的 helper。
- 它的順序固定是三段式：先確認 `.env.playwright` 是否存在，再用 `playwright-env-loader.cjs` 檢查 required keys，最後才執行登入驗證。
- 若 `.env.playwright` 不存在，先建立檔案並 `BLOCKED`，提示使用者填寫。
- 若缺少 required keys，先把缺 key 補成 `KEY=` placeholder，再 `BLOCKED`，提示使用者補值。
- 只有在 required keys 已齊全後，才會用同一套 CJS preload 做登入驗證。
- 驗證成功：控制權回到原本 step 繼續執行。
- 驗證失敗：workflow 進入 `BLOCKED`，並停在原本的 step，不會跳到下一步。
- 若 `RunReport` 已記錄 `Requires Login: true` 且 Step 5 尚未完成，重新執行時應直接接續 `/subworkflow-e2e-auth`，而不是重做文件判讀或重新判定登入需求。

## Step 6 掃站規則

- Step 6 必須拿 `testing-artifact/handoff/DocsBaseline.md` 當對照基線，不可脫離文件重新自由探索。
- 掃站至少要整理：實際入口、可達頁面、阻塞頁面、主流程節點、關鍵互動元件、與文件不一致之處、可自動化測試的候選目標。
- 若文件宣稱存在某功能或頁面，但掃站找不到，必須在 `Document Mismatches` 記錄，而不是默默忽略。
- 若掃站觀察到額外風險，例如 unstable selector、非同步載入、權限切換、資料相依，必須回寫給 Step 7 使用。
- 若需要登入，只能沿用 Step 5 `/subworkflow-e2e-auth` 已驗證通過的同一個 `testing-artifact/scripts/playwright-env-loader.cjs`。

## Step 7 測試設計規則

- Step 7 必須同時使用 `DocsBaseline`、`SrcScanSummary` 與 `SiteScanSummary`，不可只依賴其中一者。
...
- Step 8 若需要登入，執行前必須再次透過 `/subworkflow-e2e-auth` 確認登入條件仍成立；實際測試執行必須使用與 Step 5 相同的 CJS preload loader。
- Step 8 的執行摘要需標示已執行 case ID、成功與失敗案例、主要錯誤原因與重現線索，並保留可直接回填 Step 9 的案例逐項結果。
- Step 9 的最終報告需回扣 `TestPlan`、`TestCases` 與 `ExecutionSummary`，明確區分已覆蓋、未覆蓋與受阻範圍，且不可只提供 summary，必須逐項列出每個 case 的結果與證據。
...
- 若需要登入，Step 5 判定與 Step 8 執行需使用同一個 CJS preload loader（`testing-artifact/scripts/playwright-env-loader.cjs`），避免判定與正式執行結果不一致。
- `testing-artifact/scripts/playwright-env-loader.cjs` 的責任只有載入 `.env.playwright` 並輸出 required / loaded / missing key metadata；登入成功與否必須由後續登入驗證步驟判定。
- 若需要登入，必須將 key-level metadata 回寫到 `RunReport`：`Env Loaded Keys (Last Check)`、`Env Missing Required Keys (Last Check)`、`Env Validation Status (Last Check)`。
- `Env Validation Status (Last Check)` 建議值：`not_checked`、`pending_auth_validation`、`not_required`、`pass`、`fail`。
...
- `testing-artifact/handoff/SrcScanSummary.md`: 整理程式碼中的路由、權限、驗證規則、流程分支、測試線索與文件差異，作為登入判定與測試設計補充基線。
...
| 4 | `testing-artifact/handoff/RunReport.md`、`testing-artifact/handoff/InputSummary.md`、`testing-artifact/handoff/DocsBaseline.md` | `testing-artifact/handoff/SrcScanSummary.md` | `.opencode/template/SrcScanSummary.template.md` | 可實際掃描或記錄 skipped，供 Step 5-7 參考 |
| 5 | `testing-artifact/handoff/RunReport.md`、`testing-artifact/handoff/InputSummary.md`、`testing-artifact/handoff/DocsBaseline.md`、`testing-artifact/handoff/SrcScanSummary.md` | `RunReport` 的 `Requires Login` 與 env validation metadata | 無獨立檔案模板 | 若需要登入，需在同一步內委派 `/subworkflow-e2e-auth`；若因憑證缺失而 `BLOCKED`，恢復執行時必須直接從 Step 5 auth 驗證接續 |
| Auth helper | `testing-artifact/handoff/RunReport.md` | `testing-artifact/deliverables/.env.playwright`、`testing-artifact/scripts/playwright-env-loader.cjs`、`RunReport` auth metadata | `.opencode/template/.env.playwright.template`、`.opencode/template/PlaywrightEnvLoader.template.cjs` | 不佔獨立 step；僅在 Step 5 或 Step 8 且 `Requires Login = true` 時被呼叫 |
| 6 | `testing-artifact/handoff/RunReport.md`、`testing-artifact/handoff/DocsBaseline.md`、`testing-artifact/handoff/SrcScanSummary.md` | `testing-artifact/handoff/SiteScanSummary.md` | `.opencode/template/SiteScanSummary.template.md` | 必須對照 DocsBaseline，記錄可達頁面、關鍵流程、差異與自動化候選節點 |
| 7 | `testing-artifact/handoff/RunReport.md`、`testing-artifact/handoff/DocsBaseline.md`、`testing-artifact/handoff/SrcScanSummary.md`、`testing-artifact/handoff/SiteScanSummary.md` | `testing-artifact/deliverables/TestPlan.md`、`testing-artifact/deliverables/TestCases.md`、`testing-artifact/scripts/TestScript.ts` | `.opencode/template/TestPlan.template.md`、`.opencode/template/TestCases.template.md`、`.opencode/template/TestScript.template.ts` | Step 8 的固定輸入來源；案例需有來源追溯，腳本需對應 case ID 與明確 assertion |
| 8 | `testing-artifact/handoff/RunReport.md`、`testing-artifact/deliverables/TestPlan.md`、`testing-artifact/deliverables/TestCases.md`、`testing-artifact/scripts/TestScript.ts` | `testing-artifact/handoff/ExecutionSummary.md` | `.opencode/template/ExecutionSummary.template.md` | 若需要登入：先重用 `/subworkflow-e2e-auth` 驗證，再以同一 loader 執行測試 |
| 9 | `testing-artifact/handoff/RunReport.md`、`testing-artifact/handoff/ExecutionSummary.md`、`testing-artifact/handoff/ExecutionRaw.json`、`testing-artifact/deliverables/TestPlan.md`、`testing-artifact/deliverables/TestCases.md` | `testing-artifact/deliverables/TestReport.md` | `.opencode/template/TestReport.template.md` | 最終產出報告，需說明已覆蓋、未覆蓋、阻塞範圍與殘餘風險，並逐項列出每筆案例結果 |

## 指令設計

| 指令 | 範圍 | 用途 |
| --- | --- | --- |
| `/workflow-e2e` | Step 0-9 | 唯一主控入口；先檢查 `testing-artifact/handoff/RunReport.md`，不存在則用 template 初始化，存在則續跑 |
| `/subworkflow-e2e-input` | Step 1-2 | 輸入檢查、補齊缺漏 |
| `/subworkflow-e2e-docs` | Step 3、Step 5 | 文件整理與登入需求判定 |
| `/subworkflow-e2e-src` | Step 4 | SRC 掃描與文件差異整理 |
| `/subworkflow-e2e-auth` | Step 5 / Step 8 helper | `.env.playwright` / loader 建立與登入條件驗證 |
| `/subworkflow-e2e-scan` | Step 6 | 站台掃描與文件差異整理 |
| `/subworkflow-e2e-generate` | Step 7 | 產出測試計畫、測試案例、測試腳本 |
| `/subworkflow-e2e-run` | Step 8 | 執行測試腳本並整理案例結果 |
| `/subworkflow-e2e-report` | Step 9 | 產出測試報告 |
...
- 開發規格驗證內容包含：流程步驟、狀態值、必要 sections、來源追溯欄位、登入 helper 與 `testing-e2e/commands/workflow-e2e.md` 是否符合 runtime 路徑規格。
