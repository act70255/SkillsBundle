---
description: Scan the target site with Playwright for step 6
subtask: true
---
...
- `target_url` 已存在
- Step 3 已完成
- Step 4 已完成 SRC 掃描（或已明確 skipped）
- Step 5 已完成登入需求判定
- 若 `Requires Login: true`，主 workflow 已在 Step 5 內完成 `/subworkflow-e2e-auth` 驗證

只處理 Step 6：使用 Playwright 進入網站，依文件與 SRC 線索掃描站台頁面。

規則：

- 讀取：`testing-artifact/handoff/RunReport.md`、`testing-artifact/handoff/DocsBaseline.md`、`testing-artifact/handoff/SrcScanSummary.md`
- 若需要登入，Playwright 必須在執行時自行載入 `testing-artifact/deliverables/.env.playwright`；不得由 agent 先讀取其內容。
- 若需要登入，僅可沿用 Step 5 `/subworkflow-e2e-auth` 已驗證通過的同一個 `testing-artifact/scripts/playwright-env-loader.cjs`。
...
- 掃描時要對關鍵互動做「互動前置檢查（interaction preflight）」：確認觸發方式、元素可互動條件、可能遮罩/動畫/loading 阻擋、觸發後穩定訊號（URL/DOM/API/Toast）與建議等待策略。
...
- `testing-artifact/handoff/SiteScanSummary.md` 需包含「互動前置檢查結果」區塊，至少列出 3 筆高風險互動（含建議 selector 與等待訊號），供 Step 7 直接生成腳本 guard。
